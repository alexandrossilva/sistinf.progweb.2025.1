...
function calcularPesoIdeal() {
    ...
    if (sexo == -1)                     // validação de entrada de sexo
        ...
        // atualização de propriedade de visualização de conteiner de botão
        document.getElementById("btnLimpar").parentNode.style.display = "flex";
    }
}

// remoção de linhas de tabela com histórico de pesos ideais calculados até então
function limparHistorico() {
    // obtenção de referência de tabela da página
    let tabHistorico = document.getElementById("tabHistorico");

    // obtenção de linhas de tabela
    let linhas = tabHistorico.children;

    // remoção de linhas da tabela excetuando-se 1ª e 2ª linhas
    while (linhas.length > 2) {
        let index = linhas[linhas.length - 1];  // índice de última linha
        tabHistorico.removeChild(index);        // remoção de última linha
    }
